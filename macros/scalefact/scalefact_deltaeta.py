...
def loop(self):
	...
	binEdges = array('f',[1.5,1.6,1.7,1.8,1.9,2.0,2.1,2.2])
	...
	plotDict = {#key = var name, value = [varType, nBins, binLow, binHigh, title]
				...
				#				 "vR", "vRF
	'deltaEta':["sF",nBins,1.5,2.2,self.fileID+"; DeltaEta; Events"]
	}
	...
	for iEvent in range(nEvents):
		...
		if (numSVJ < 0) or (numSVJ > 2):
			print("numSVJ = {}".format(numSVJ))
			continue
		histDict_numer["deltaEta"].Fill(deltaEta, weight*lumi*numSVJ)
		histDict_denom["deltaEta"].Fill(deltaEta, weight*lumi*2.0)
